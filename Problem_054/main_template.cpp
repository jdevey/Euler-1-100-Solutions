...
bool cmpHands(pair <int, char>* a, pair <int, char>* b) {
	...
	for (int i = 0; i < 5; ++i) {
		...
		bb[i] = {csb[b[i].first], csb[b[i].first]};
	}
	sort(aa, aa + 5);
	sort(bb, bb + 5);
	for (int i = 0; i < 5; ++i) {
...
int main() {
	...
	for (int i = 0; i < 1000; ++i) {
		...
		for (int j = 0; j < 5; ++j) {
			string in;
			fin >> in;
			if (in[0] >= '2' && in[0] <= '9') a[i].first = in[0] - '0';
			else for (int k = 0; k < 5; ++k) if ("TJQKA"[k] == in[0]) a[i].first == 10 + k;
			a[i].second = in[1];
		}
		for (int j = 0; j < 5; ++j) {
			string in;
			fin >> in;
			if (in[0] >= '2' && in[0] <= '9') b[i].first = in[0] - '0';
			else for (int k = 0; k < 5; ++k) if ("TJQKA"[k] == in[0]) b[i].first == 10 + k;
			b[i].second = in[1];
		}
